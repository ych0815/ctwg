...
void BSP_Init (void)
{
    ...
    BSP_SetECT_Prescaler(128);                                    /* Set ECT prescaler to prevent OSTickCnts overrflow    */    
...
#else     
    SYNR   = (0x13              |                                /* Set multiplier of 20                                 */
            SYNR_VCOFRQ0_MASK                                    /* VCORANGE: 80MHz < Fvco < 120MHz                      */
            );
...
CPU_INT32U  BSP_CPU_ClkFreq (void)
{
    ...
    posdiv = 1;
#else
    posdiv = (POSTDIV & 0x1F) * 2;
#endif    

    if ((CLKSEL & CLKSEL_PLLSEL_MASK) > 0) {
        clk_freq =  ((OSCFREQ * 2) * mul) / (div * posdiv);
    } else {
        clk_freq =    OSCFREQ;
    }    
    return  (clk_freq);
...
static void  OSTickISR_Init (void)
{
    ...
    OSTickCnts    = (CPU_INT16U)((bus_frq /ect_prescaler/OS_TICKS_PER_SEC) - 1); //modify  by sunj 2019/1/4 16:42
